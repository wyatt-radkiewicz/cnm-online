...
void GameState_MainMenu_Init(void)
{
	...
	Util_SetRect(wings + 0, 384, 352, 48, 48);
	Util_SetRect(wings + 1, 448, 352, 48, 48);
	controlling = 0;
	Util_RandSetSeed(0);
...
void GameState_MainMenu_Draw(void)
{
	...
	Renderer_DrawBitmap((int)pos[0], (int)pos[1], &player, 0, RENDERER_LIGHT);
	Renderer_DrawBitmap((int)pos[0] - 8, (int)pos[1] - 8, wings + (Game_GetFrame() / 10 % 2), 5, RENDERER_LIGHT);
	Renderer_DrawBitmap(160 - 96, 30, &title_card, 0, RENDERER_LIGHT);
	Gui_Draw();
	GameConsole_Draw();
	Renderer_Update();
